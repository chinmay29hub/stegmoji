...
export async function decode(encodedText, options = {}) {
  ...
  if (mode === 'auto') {
    ...
    if (allAtEnd) {
      detectedMode = MODES.TAIL
    } else {
      // For non-tail modes, default to INTERLEAVED
      // Both INTERLEAVED and ZWJ_AWARE use the same extraction method
      detectedMode = MODES.INTERLEAVED
    }
  }
  ...
  switch (detectedMode) {
    ...
    case MODES.ZWJ_AWARE: {
      // For ZWJ-aware mode, we need to extract VS based on grapheme cluster boundaries
      // This is more complex, but for now we'll use the same method as interleaved
      // since both modes insert VS in sequence
      variationSelectors = encodedText
        .split('')
        .filter(char => char === VS15 || char === VS16)
        .join('')
  ...
  const paddedBits = bits.length % 8 === 0 ? bits : bits.padEnd(Math.ceil(bits.length / 8) * 8, '0')
  
  // Convert bits to bytes
  const payload = bitsToBytes(paddedBits)
